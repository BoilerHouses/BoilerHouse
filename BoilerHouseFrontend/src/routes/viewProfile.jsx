...
const ViewProfile = () => {
  ...
  if (isLoading) {
    ...
    return (
      <div className={recommendedUsers[userId] >= threshold ? `relative border rounded-lg p-6 max-w-full mx-auto bg-yellow-${200 + (100 * Math.round(Math.round( ((recommendedUsers[userId] - threshold) * 700)) / 100))} shadow-md` : "relative border rounded-lg p-6 max-w-full mx-auto bg-gray-100 shadow-md"}>
        {/* Edit Profile Button */}
        <button
          className={
        ...
        <p className={userId in recommendedUsers ? "italic mb-4" : "hidden"}>{`Your similarity score is: ${(recommendedUsers[userId] * 100).toFixed(2)}`}</p>
